...
annualMean <- all %>%
  group_by(city, year) %>%
  summarise(mean(meanTemp, na.rm = TRUE))

low_and_mean <- merge(count23, annualMean)
...
p + geom_point(aes(colour = city), size = 4) +
  ...
  labs(x = "Mean annual temperature (°C)",
       y = "Number of days with low temperature > 23°C (73°F)") +
  geom_text(hjust= -0.2, vjust=0, size = 4, aes(colour = city)) +
  guides(colour = guide_legend(title=NULL)) +
  theme_minimal(base_size = 22)
...
p <- ggplot(data = meanLow23, aes(x = order.city, y = average, label = round(average)))
p + geom_point(size = 4) +
...
notHot <- filter(all, low <= 23 & high >= 30)
relief <- filter(all, low <= 21 & high >= 30)
mild <- filter(all, low <= 18 & high >= 30)

mildSummary <- mild%>%
  group_by(city, year) %>%
  summarise((length(low)))

colnames(mildSummary) <- c("city", "year", "count")

# plot this as city & year for low temps > 23C & high >= 30
p <- ggplot(data = mildSummary, aes(x = year, y = count))
p + geom_point(aes(colour = city), size = 4) +
  geom_line(aes(colour = city), size = 0.2) +
  scale_colour_brewer(palette = "Set1",
                      drop = TRUE, limits = levels(mildSummary$city)) +
  scale_y_continuous(limits = c(0, max(mildSummary$count))) +
  labs(x = "Year",
       y = "# of days with low <= 18°C & high >= 30°C, 2010 to 2014") +
  guides(colour = guide_legend(title=NULL)) +
  theme_minimal(base_size = 22)



hot <- filter(all, city == "Carefree, AZ, USA" &
                high >= 40)
